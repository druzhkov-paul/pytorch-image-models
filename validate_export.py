...
def validate(args):
    ...
    if args.num_classes is None:
        ...
        args.num_classes = pt_model.num_classes

    param_count = sum([m.numel() for m in pt_model.parameters()])
    _logger.info('Model %s created, param count: %d' % (args.model, param_count))

    data_config = resolve_data_config(vars(args), model=pt_model, use_test_size=True)
    ...
    with torch.no_grad():
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            top1.update(acc1.item(), input.shape[0])
            top5.update(acc5.item(), input.shape[0])
